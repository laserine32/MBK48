					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							...
							>
								...
								<div className="grid flex-1 text-left text-sm leading-tight">
									<span className="truncate font-semibold">{capitalizeFirstLetter(activeTheme.name)}</span>
									<span className="truncate text-xs">{"Theme"}</span>
								</div>
								<ChevronsUpDown className="ml-auto" />
							</SidebarMenuButton>
						...
						>
							<DropdownMenuLabel className="text-xs text-muted-foreground">Theme</DropdownMenuLabel>
							{themeItem.map((theme) => (
								<DropdownMenuItem key={theme.name} onClick={() => setActiveTheme(theme)} className="gap-2 p-2">
									<div className="flex size-6 items-center justify-center rounded-sm border">
