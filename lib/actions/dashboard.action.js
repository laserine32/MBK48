import { packInUseModel, packModel, productionModel, purchaseModel } from "../repo"
...
export const fetchDataCards = async () => {
	...
	return {
		expenses: {
			total: formatCurrency(fexpenses),
...
const getLastPurchase = async () => {
	const data = await purchaseModel.tbl.findFirst({
		orderBy: [
			{
				date: "desc",
			},
	...
	if (!data) return ""
	return formatDateToLocal(data.date)
}
...
const getCountPack = async () => {
	const data = await packModel.tbl.aggregate({
		_count: {
			id: true,
		},
		where: {
...
export const getPackRatioChart = async () => {
	const fproduced = await getProduction()
	const fpackinuse = await getPackInUse()
	const max = Math.max(fproduced, fpackinuse.total)
	const min = Math.min(fproduced, fpackinuse.total)
	const gap = max - min
	const total = fproduced + fpackinuse.total
	// const vpro = (fproduced / total) * 100
	// const vinu = (fpackinuse.total / total) * 100
	const pack = await getCountPack()
	const vpro = pack - gap - 1
	const vinu = pack - vpro
	console.log(pack)
	// console.log(max, min, gap, total, vpro, vinu)
	return {
		chartData: [
			{ item: "Packed", value: vpro, total: fproduced, fill: "hsl(var(--chart-2))" },
			{ item: "Used", value: vinu, total: fpackinuse.total, fill: "hsl(var(--chart-3))" },
		],
		gap: gap,
	}
}
...
const mergeMultipleObjects = (lists) => {
	...
	return uniqueMonths.map((tgl) => {
		...
		lists.forEach((list, index) => {
			...
			if (matchingItem) {
				lastMonth = matchingItem[key[1]]
			}
			mergedItem[key[1]] = matchingItem ? matchingItem[key[1]] : lastMonth
